...
class BetterEvalAgentTest(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(BetterEvalAgentTest, self).__init__(question, testDict)
        self.layoutName = testDict['layoutName']
        self.agentName = testDict['agentName']
        self.ghosts = eval(testDict['ghosts'])
        self.maxTime = int(testDict['maxTime'])
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        def gradeThreshold(value, minimum, thresholds, name, point_step):
            points = 0
            passed = (minimum == None) or (value >= minimum)
            if passed:
                for t in thresholds:
                    if value >= t:
                        points += point_step
            return (passed, points, value, minimum, thresholds, name, point_step)

        results = [gradeThreshold(averageScore, self.scoreMinimum, self.scoreThresholds, "average score", 1),
                   gradeThreshold(nonTimeouts, self.nonTimeoutMinimum, self.nonTimeoutThresholds, "games not timed out", 13),
                   gradeThreshold(wins, self.winsMinimum, self.winsThresholds, "wins", 2)]

        totalPoints = 0
        for passed, points, value, minimum, thresholds, name, point_step in results:
            if minimum == None and len(thresholds)==0:
                continue
            ...
            if not passed:
                ...
                self.addMessage("%s %s (%s of %s points)" % (value, name, points, len(thresholds)*point_step))
            ...
            if minimum != None:
                ...
                if len(thresholds)==0 or minimum != thresholds[0]:
                    self.addMessage("    >= %s:  0 points" % (minimum,))
                for idx, threshold in enumerate(thresholds):
                    self.addMessage("    >= %s:  %s points" % (threshold, point_step*(idx+1)))
            elif len(thresholds) > 0:
                self.addMessage("    Grading scheme:")
                self.addMessage("     < %s:  0 points" % (thresholds[0],))
                for idx, threshold in enumerate(thresholds):
